...
tokens <- tokens(corpus, what= "word") # transform corpus to tokens 

# ------------------------------------------------------------------------------
# Build n-gram frequencies
# ------------------------------------------------------------------------------
# get top three duplicated word in n gram data frame

getTopThree <- function(corpus) {
  first <- !duplicated(corpus$token) #get logical value for non duplicated token0
  balance <- corpus[!first,] #save duplicated values in balance
  first <- corpus[first,] #save non duplicated values in first
  second <- !duplicated(balance$token) #select duplicated from balance dataset
...
tokenFrequency <- function(tokens, n = 1, rem_stopw = NULL) {
  tokens <- tokens_ngrams(tokens,n= n)
  tokens <- dfm(tokens)
  tokens <- colSums(tokens)
  total <- sum(tokens)
  tokens <- data.frame(names(tokens),
                       tokens,
                       row.names = NULL,
                       check.rows = FALSE,
                       check.names = FALSE,
  ...
  colnames(tokens) <- c("token", "n")
  tokens <- mutate(tokens, token = gsub("_", " ", token))
  tokens <- mutate(tokens, percent = tokens$n / total)
  tokens <- tokens[tokens$n > 1 ,]
  tokens$outcome <- word(tokens$token, -1) #select the last word in n gram
  tokens$token <- word(string = tokens$token, start = 1, end = n - 1, sep = fixed(" ")) #select the first n-1 words from n grams
  setorder(tokens, -n)
  tokens <- getTopThree(tokens)
  return(tokens)
}
...
startWordPrediction <- startWord$outcome[1:3]  # select top 3 words to start word prediction app
saveRDS(startWordPrediction, "start-word-prediction2.RData")

# bigram
bigram <- tokenFrequency(tokens, n = 2, NULL)
saveRDS(bigram, "bigram2.RData")
remove(bigram)

# trigram
trigram <- tokenFrequency(tokens, n = 3, NULL)
trigram <- trigram %>% filter(n > 1)
saveRDS(trigram, "trigram2.RData")
remove(trigram)

# quadgram
quadgram <- tokenFrequency(tokens, n = 4, NULL)
